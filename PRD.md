...
**문제**: 9! (362,880가지) 라인업에 대해 각 10,000회 시뮬레이션을 수행하는 것은 클라이언트 성능 한계를 초과합니다.
**해결 방안**:
1.  **유전 알고리즘 (Genetic Algorithm) 도입**:
    -   초기 랜덤 라인업 집단(Population) 생성.
    -   우수 라인업 선택(Selection) 및 교차(Crossover/Mutation)를 통한 세대 진화.
    -   전수 조사 대신 국소 최적해(Local Optima)를 빠르게 탐색하여 2초 이내 결과 도출.
2.  **Web Worker 활용**: 메인 UI 스레드와 분리된 별도 스레드에서 연산 수행.
