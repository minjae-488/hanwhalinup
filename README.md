...
-   **⚾ 정교한 시뮬레이션 엔진**: 선수별 세부 스탯(AVG, OBP, SLG 등)을 기반으로 실제 야구 경기 로직을 구현한 **몬테카를로 시뮬레이션**을 수행합니다.
-   **📊 실시간 라인업 최적화**: 36만 가지 이상의 가능한 타순 조합 중, 휴리스틱 알고리즘을 통해 기대 득점을 최대화하는 'Best Lineup'을 찾아냅니다.
